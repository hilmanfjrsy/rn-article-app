...
function InteractiveChart({ dateList = [], priceList = [], title = "Statistik" }) {
  const apx = (size = 0) => {
    let width = Dimensions.get('window').width - 20;
    return (width / 750) * size;
  };

  // const [dateList, setDateList] = useState(allDate);
  // const [priceList, setPriceList] = useState(allValue);
  ...
  const CustomGradient = () => (
    <Defs key="gradient">
      <LinearGradient id="gradient" x1="0" y="0%" x2="0%" y2="100%">
        ...
        <Stop offset="0%" stopColor={GlobalVar.primaryColor} stopOpacity={0.25} />
        <Stop offset="100%" stopColor={GlobalVar.primaryColor} stopOpacity={0} />
      </LinearGradient>
    </Defs>
  );
      ...
      <G x={x(positionX)} key="tooltip">
        <G
          ...
          <Rect
            ...
            width={apx(300)}
            height={apx(96)}
            stroke={GlobalVar.primaryColor + 50}
  ...
  return (
    <View
      ...
      <Text style={[GlobalStyles.fontTitle, { color: GlobalVar.blackColor, marginBottom: 20, textAlign: 'center' }]}>{title}</Text>
      <View
        style={{
          flexDirection: 'row',
          width: apx(750),
